...
expName = '2back'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
                                 ...
                                 originPath='/Users/howardhsu/Desktop/pychopy/2back project/2back.py',
...
TypeControl_list = ['0'] * 128 + ['1'] * 32  # 0 is nontarget; 1 is target
random.shuffle(TypeControl_list)
...
for thisBlockloop in blockloop:
    ...
    # set up handler to look after randomisation of conditions etc
    target_loop = data.TrialHandler(nReps=2, method='random',
                                    extraInfo=expInfo, originPath=-1,
                                    trialList=[None],
    ...
    # completed 2 repeats of 'target_loop'

    # set up handler to look after randomisation of conditions etc
    twoback_loop = data.TrialHandler(nReps=2, method='random',
                                     extraInfo=expInfo, originPath=-1,
                                     trialList=[None],
                                     seed=None, name='twoback_loop')
    thisExp.addLoop(twoback_loop)  # add the loop to the experiment
    thisTwoback_loop = twoback_loop.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTwoback_loop.rgb)
    if thisTwoback_loop != None:
        for paramName in thisTwoback_loop:
            exec('{} = thisTwoback_loop[paramName]'.format(paramName))

    for thisTwoback_loop in twoback_loop:
        currentLoop = twoback_loop
        # abbreviate parameter names if possible (e.g. rgb = thisTwoback_loop.rgb)
        if thisTwoback_loop != None:
            for paramName in thisTwoback_loop:
                exec('{} = thisTwoback_loop[paramName]'.format(paramName))
        ...
        if TypeControl_list[i - 2] == '1':  # target
            stimulus = stimulus_list[i - 2]
            corResponse = 'z'
            i += 1
            stimulus_list.append(stimulus)
        elif TypeControl_list[i - 2] == '0':  # nontarget
        ...
        # keep track of which components have finished
        routine_2_backComponents = [fixation, question, response]
        ...
        if TypeControl_list[0] == '1':  # target
            IsTwoBack = 1
            thisExp.addData('IsTwoBack', IsTwoBack)
        elif TypeControl_list[0] == '0':  # nontarget
            IsTwoBack = 0
            thisExp.addData('IsTwoBack', IsTwoBack)
        # check responses
        ...
        # store data for twoback_loop (TrialHandler)
        twoback_loop.addData('response.keys', response.keys)
        twoback_loop.addData('response.corr', response.corr)
        if response.keys != None:  # we had a response
            twoback_loop.addData('response.rt', response.rt)
    ...
    # completed 2 repeats of 'twoback_loop'
